...
const MedicineAppearance = ({item, size = 'small'}) => {
  ...
  // 색상 분리 처리
  const colors = item.color_classes
    ? item.color_classes.split(',').map(color => color.trim())
    : [];
...
const AppearanceLabel = styled.Text`
  ...
  font-size: ${({fontSizeMode, size}) =>
    size === 'large'
      ? FontSizes.body[fontSizeMode]
      : FontSizes.caption[fontSizeMode]};
`;
