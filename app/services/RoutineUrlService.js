...
const findClosestRoutineByTime = (routineGroups) => {
  ...
  console.log('[RoutineUrlService] 입력 데이터:', JSON.stringify(routineGroups));
  ...
  if (!todayGroup) {
    ...
    console.log('[RoutineUrlService] 사용 가능한 날짜:', routineGroups.map(g => g.take_date).join(', '));
    return null;
  }
  
  console.log('[RoutineUrlService] 오늘 루틴 그룹 찾음:', JSON.stringify(todayGroup));
  ...
  if (closestSchedule) {
    ...
    console.log('[RoutineUrlService] 결과 루틴 정보:', JSON.stringify(result));
    return result;
  }
...
const formatRoutineForDisplay = (routineInfo) => {
  console.log('[RoutineUrlService] 모달용 데이터 포맷팅 시작:', JSON.stringify(routineInfo));
  ...
  console.log('[RoutineUrlService] 포맷팅 결과:', JSON.stringify(result));
  return result;
};

// URL 스킴 처리 함수
const handleUrlScheme = async (url) => {
  console.log('[RoutineUrlService] URL 스킴 처리 요청 받음:', url);
  ...
  // ⚠️ 중요: 중복 처리 방지 로직 비활성화 (디버깅용)
  // const now = Date.now();
  // if (url === lastProcessedUrl && now - lastProcessTime < 10000) {
  //   console.log('[RoutineUrlService] 최근 처리된 URL 무시:', url);
  //   return;
  // }
  
  // ⚠️ 새 로직: 이미 모달이 열려있으면 닫기 (모달 초기화)
  if (isModalVisible) {
  ...
  if (url && url.includes('medeasy://openroutine')) {
    try {
      ...
      console.log('[RoutineUrlService] API 응답 받음:', JSON.stringify(response));
      
      if (!response.data || response.data.result.result_code !== 200 || !response.data.body) {
        console.error('[RoutineUrlService] 루틴 정보 가져오기 실패:', JSON.stringify(response));
      ...
      if (!closestRoutineInfo || closestRoutineInfo.routines.length === 0) {
        console.log('[RoutineUrlService] 현재 시간에 해당하는 복용할 약이 없음');
        Alert.alert('알림', '현재 시간에 해당하는 복용할 약이 없습니다.');
        return;
      }
      
      console.log('[RoutineUrlService] 현재 시간에 해당하는 복용할 약 찾음:', JSON.stringify(closestRoutineInfo));
      ...
      try {
        ...
        console.log('[RoutineUrlService] 복용 체크 응답 결과:', JSON.stringify(checkResults));
        
        // 모든 체크가 성공했는지 확인
        const allSuccessful = checkResults.every(result => 
          result.data && result.data.result && result.data.result.result_code === 200
        );
        
        if (!allSuccessful) {
          console.error('[RoutineUrlService] 일부 루틴 복용 체크 실패:', JSON.stringify(checkResults));
      ...
      // ⚠️ 중요: 중복 처리 방지 로직 수정
      // 마지막 처리 URL과 시간 저장 비활성화 (디버깅용)
      // lastProcessedUrl = url;
      // lastProcessTime = Date.now();
      
      console.log('[RoutineUrlService] 모달 표시 상태 변경:', isModalVisible);
      console.log('[RoutineUrlService] 현재 루틴 데이터:', JSON.stringify(currentRoutineData));
...
const addListener = (callback) => {
  ...
  console.log('[RoutineUrlService] 새 리스너에게 현재 상태 전달:', JSON.stringify(currentState));
  ...
  return () => {
    ...
    if (index !== -1) {
      ...
      console.log('[RoutineUrlService] 제거할 리스너를 찾지 못함');
    }
  };
};
