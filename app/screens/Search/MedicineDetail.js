...
const MedicineDetailScreen = ({ navigation }) => {
  ...
  const medicine = {
    ...
    se_qesitm: '만성 녹내장을 악화시킬 경우 복용을 즉각 중지하고 의사 또는 약사와 상의하십시오.',
    deposit_method_qesitm: '실온에서 보관하십시오.어린이의 손이 닿지 않는 곳에 보관하십시오.',
  };
      ...
      <ScrollView>
        ...
        >
            ...
            <View style={{flexDirection: 'row', gap: 11, marginTop: 15}}>
                <Tag sizeType='large' colorType='detailPrimary'>{medicine.etc_otc_name}</Tag>
                <Tag sizeType='large' colorType='detailSecondary'>{medicine.class_name}</Tag>
            </View>
        </MedicineInfoContainer>

        <MedicineDetailContainer>
            <MedicineAppearanceContainer>
                <View style={{backgroundColor: themes.light.boxColor.inputPrimary, padding: 10, gap: 8, borderRadius: 10, }}>
                  ...
                  <Appearance label={'크기       '} value={`${medicine.leng_long} X ${medicine.leng_short} X ${medicine.thick} (mm)`}/>
                </View>
            </MedicineAppearanceContainer>

            <MedicineUsageContainer>
              <View style={{paddingTop: 10, borderBottomWidth: 10, borderBottomColor: themes.light.borderColor.borderSecondary, }}>
                <Usage label={'💊 이런 증상에 효과가 있어요'} value={medicine.efcy_qesitm}/>
                <Usage label={'📋 이렇게 복용하세요'} value={medicine.use_method_qesitm}/>
                <Usage label={'🗄️ 이렇게 보관하세요'} value={medicine.deposit_method_qesitm}/>
              </View>
              <View style={{borderBottomWidth: 10, borderBottomColor: themes.light.borderColor.borderSecondary, }}>
                <Usage label={'⚠️ 이런 주의사항이 있어요'} value={medicine.atpn_qesitm}/>
                <Usage label={'🤒 이런 부작용이 예상돼요'} value={medicine.se_qesitm}/>
              </View>
            </MedicineUsageContainer>
            <SimilarMedicinesContainer>
              <HeadingText style={{paddingHorizontal: 20}}>비슷한 약 보기</HeadingText>
...
const Appearance = ({ label, value }) => (
  <View style={{ flexDirection: 'row', gap: 18 }}>
    ...
    >{label}</Text>
    <Text style={{ color: themes.light.pointColor.Primary,
...
const SimilarMedicineItem = ({ item }) => (
  <View style={{ marginRight: 15, width: 138.75}}>
    <Image source={{uri: item.item_image}} style={{ width: 138.75, height: 74, borderRadius: 10, }} />
    <View style={{marginTop: 15, gap: 8}}>
      ...
      ellipsizeMode="tail" 
      >{item.item_name}</Text>
      <Tag sizeType='small' colorType='resultPrimary'>{item.class_name}</Tag>
    </View>
  </View>
);
